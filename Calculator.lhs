...
We could have used the `type` keyword to define a type synonym.
But after defining a few simple basic
...
Note that `[]` represents a failed parse, and that 'tok' expects the correct
...
> run :: (Functor m, Monad m) => Functions -> Tree -> StateT Vars (ErrorT String m) Double
> run funs = go
>   where
...
> run' :: Functions             -- ^ static function symbol table
>      -> (String -> r)         -- ^ error continuation
>      -> Tree                  -- ^ tree to evaluate
>      -> Vars                  -- ^ (dynamic) variable symbol table
>      -> (Vars -> Double -> r) -- ^ success continuation
>      -> r
> run' funs err = go
>   where
>     go (UMinus t)       s k = go t s (\ s' r -> k s' (negate r))
>     go (Num n)          s k = k s n
>     go (Var x)          s k = case M.lookup x s of
>                                 Just v  -> k s v
>                                 Nothing -> err $ "unknown variable: " ++ x
>     go (Assign x t)     s k = go t s (\ s' r -> k (M.insert x r s') r)
>     go (BinOp op t1 t2) s k = go t1 s (\ s' r1 -> go t2 s (\ s'' r2 -> k s'' (runOp op r1 r2)))
>     go (Call f t)       s k = case M.lookup f funs of
>                                 Just g  -> go t s (\ s' r -> k s' (g r))
>                                 Nothing -> err $ "unknown function: " ++ f
...
> runInteractive :: IO ()
> runInteractive = runErrorT (runStateT loop vars) >> return ()
>   where
>     loop :: StateT Vars (ErrorT String IO) ()
>     loop = do
>       r <- liftIO getLine
>       unless (L.null r) $ do
>         case toTree r of
>           Nothing -> liftIO (putStrLn "invalid input")
>           Just t  -> catchError (run functions t >>= liftIO . putStrLn . ("  "++) . show)
>                                 (liftIO . putStrLn)
>         loop
...
>           Just t  -> run' functions errk t s resk
>       where
>         errk :: String -> IO ()
>         errk msg = do
>           putStrLn msg
...
take a list of input lines and return the result of all the expressions,
aborting if there's any invalid input in between:

TODO: fix handling of invalid inputs

> runBatch :: [String] -> Maybe [Double]
> runBatch xs =
>   either (const Nothing) (Just . fst) $
>   runIdentity (runErrorT (runStateT (mapM (run functions) (catMaybes (L.map toTree xs))) vars))
...
> testsRun =
>   test [
>     "run1" ~: runBatch ["x = 2", "y = 3", "x = x + y"] ~?= Just [2, 3, 5],
>     "run2" ~: runBatch ["x = y = log(e)", "x", "y"]    ~?= Just [1, 1, 1],
>     "run3" ~: runBatch ["x = 2", "x = x + y", "x"]     ~?= Nothing
>   ]
